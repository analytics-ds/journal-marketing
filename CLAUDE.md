...
- Covers d'articles : **cascade `Pexels` -> `Unsplash` -> `Openverse` -> visuel de charte genere** (WebP dans `static/images/blog/`, champ `imageCredit` OBLIGATOIRE des que le script renvoie un credit). Script : `.claude/scripts/fetch-image.sh "<query en anglais>" "<slug>"`. Les cles `PEXELS_API_KEY` et `UNSPLASH_ACCESS_KEY` viennent du **prompt de la routine**, jamais du repo (public). **Openverse est passe en dernier recours le 2026-09-04** : sur les 45 heros du blog il n'avait produit que 15 photos pertinentes, contre 10 franchement hors sujet, 15 generiques et 5 degrades de secours. **Toujours verifier visuellement (Read) chaque image telechargee avant integration**, quelle que soit la banque. Avatars auteurs : SVG monogrammes dans `static/images/authors/`.
...
- **`roadmap.yaml`** à la racine : 50 mots-clés sur les 7 rubriques, du 2026-08-06 au 2026-10-14. **5 articles par semaine, du lundi au vendredi**, un par jour ouvré. Le démarrage se fait un jeudi, la première semaine ne porte donc que 2 articles.
- **Rotation en cycle continu** : SEO, Contenu, Publicité en ligne, IA et GEO, Data et mesure, Social et influence, Outils et comparatifs, puis on reboucle. Le cycle **enjambe les semaines** et ne se remet jamais à zéro le lundi : avec 5 jours ouvrés pour 7 rubriques, une semaine ne couvre jamais tout le site, la suivante reprend là où on s'était arrêté. Chaque rubrique revient tous les 7 articles. Deux articles consécutifs ne sont jamais de la même rubrique. En rechargeant la roadmap, poursuivre le cycle, ne pas le recaler sur le lundi.
